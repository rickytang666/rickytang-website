export interface TechSkill {
  name: string;
  icon: string;
  alt: string;
}

export const languages: TechSkill[] = [
  {
    name: "C++",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=cpp&theme=dark",
    alt: "c++"
  },
  {
    name: "Python",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=python&theme=dark",
    alt: "python"
  },
  {
    name: "Processing",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=processing&theme=dark",
    alt: "processing"
  },
  {
    name: "JavaScript",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=javascript&theme=dark",
    alt: "javascript"
  },
  {
    name: "TypeScript",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=typescript&theme=dark",
    alt: "typescript"
  },
  {
    name: "HTML",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=html&theme=dark",
    alt: "html"
  },
  {
    name: "CSS",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=css&theme=dark",
    alt: "css"
  },
  {
    name: "LaTeX",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=latex&theme=dark",
    alt: "latex"
  },
];

export const tools: TechSkill[] = [
  {
    name: "React",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=react&theme=dark",
    alt: "react"
  },
  {
    name: "Next.js",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=nextjs&theme=dark",
    alt: "nextjs"
  },
  {
    name: "Node.js",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=nodejs&theme=dark",
    alt: "nodejs"
  },
  {
    name: "Express",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=express&theme=dark",
    alt: "express"
  },
  {
    name: "PostgreSQL",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=postgresql&theme=dark",
    alt: "postgresql"
  },
  {
    name: "Supabase",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=supabase&theme=dark",
    alt: "supabase"
  },
  {
    name: "Tailwind CSS",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=tailwind&theme=dark",
    alt: "tailwind"
  },
  {
    name: "DaisyUI",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=daisyui&theme=dark",
    alt: "daisyui"
  },
  {
    name: "shadcn/ui",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=shadcn&theme=dark",
    alt: "shadcn"
  },
  {
    name: "Git",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=git&theme=dark",
    alt: "git"
  },
  {
    name: "GitHub",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=github&theme=dark",
    alt: "github"
  },
  {
    name: "Vercel",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=vercel&theme=dark",
    alt: "vercel"
  },
  {
    name: "Figma",
    icon: "https://go-skill-icons.vercel.app/api/icons?i=figma&theme=dark",
    alt: "figma"
  },
]; 